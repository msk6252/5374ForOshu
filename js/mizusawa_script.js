...
/*   var descriptions = new Array(); */
  ...
  function updateAreaList() {
    csvToArray("data/area_days/mizusawa_days.csv", function(tmp) {
      ...
      csvToArray("data/center/center.csv", function(tmp) {
        //ゴミ処理センターのデータを解析します。
        //表示上は現れませんが、
  ...
  function createMenuList(after_action) {
    // 備考データを読み込む
    csvToArray("data/remarks/mizusawa_remarks.csv", function(data) {
      data.shift();
      for (var i in data) {
        remarks.push(new RemarkModel(data[i]));
      }
    });
    csvToArray("data/description/description.csv", function(data) {
      data.shift();
      for (var i in data) {
        descriptions.push(new DescriptionModel(data[i]));
      }

      csvToArray("data/target/target.csv", function(data) {
